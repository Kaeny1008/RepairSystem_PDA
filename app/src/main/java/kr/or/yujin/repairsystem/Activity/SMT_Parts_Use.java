...
public class SMT_Parts_Use extends AppCompatActivity{
    ...
    private String activityTag = "자재투입(사용)";
    private TextView etWorker, etLotNo, etWorkMode, etORGPartNo,
            ...
            etDippingFeederNo;
    private Button btnMaterialUse;

    private String tempFeederNo = null;
    private String dippingFeeder = null;
    private String[] feederNoList = {};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_smt_parts_use);
        ...
        if (etWorkMode.getText().toString().equals("PMIC")) {
            ...
            dippingFeeder = "Solder";
        }

        getData load_DippingFeeder = new getData();
        load_DippingFeeder.execute("http://" + server_ip + ":" + server_port + "/Repair_System/SMT_Manager/load_dipping_feeder.php",
                "load_Dipping_Feeder",
                dippingFeeder);
        ...
        tvChangePartNo.setOnClickListener((new View.OnClickListener() {
            @Override
        ...
        tvChangeLotNo.setOnClickListener((new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                etChangeLotNo.setText("");
                etChangePartNo.setText("");
        ...
        tvChangeQty.setOnClickListener((new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                etChangeLotNo.setText("");
                etChangePartNo.setText("");
    ...
    private void materialUsedReg(){
        ...
        if (etDippingFeederNo.length() == 0){
            tvStatus.setText("Dipping Feeder No.를 스캔하여 주십시오.");
            return;
        }
        ...
        if (etORGPartNo.getText().equals(etChangePartNo.getText())){
            String sendText = "insert into smt_material_used_history(feeder_no, lot_no, work_section,material_part_no";
            sendText += ", material_lot_no, basic_stock_qty, material_used_qty, material_loss_qty";
            sendText += ", start_date, start_worker, check_result, dipping_feeder_check, dipping_feeder_no) values(";
            sendText += "'" + etFeederNo.getText() + "'";
            sendText += ",'" + etLotNo.getText() + "'";
            sendText += ",'" + etWorkMode.getText() + "'";
            sendText += ",'" + etChangePartNo.getText() + "'";
            ...
            sendText += ",'OK'";
            sendText += ",'OK'";
            sendText += ",'" + etDippingFeederNo.getText() + "'";
            sendText += ");";
            ...
            vibrator.vibrate(pattern, -1); // miliSecond, 지정한 시간동안 진동
            Intent intent = new Intent(SMT_Parts_Use.this, SMT_MisCheck.class);
            intent.putExtra("callActivity", "SMT_Parts_Use");
    ...
    private void InitializeControl()
    {
        ...
        etDippingFeederNo = (TextView) findViewById(R.id.etDippingFeederNo);
                    ...
                    if (ScanConst.INTENT_USERMSG.equals(intent.getAction())) {
                        ...
                        if (sResult.substring(0, 3).equals("FN-")){
                            if (etFeederNo.length() == 0){
                                ...
                                tempFeederNo = sResult;
                            } else if (etDippingFeederNo.length() == 0) {
                                Integer noFind = 0;
                                for(String str : feederNoList) {
                                    if(str.equals(sResult)) {
                                        System.out.println("Dipping Feeder 리스트에 해당피더가 존재합니다.");
                                        noFind = 1;
                                        etDippingFeederNo.setText(sResult);
                                        materialUsedReg();
                                        break;
                                    }
                                }
                                if (noFind == 0){
                                    tvStatus.setText("해당 Dipping Feeder는 사용할 수 없습니다.");
                                }
                            }
    ...
    private class getData extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            ...
            if (ActivityName.equals("Activity.SMT_Manager"))
                progressDialog = ProgressDialog.show(SMT_Parts_Use.this,
        ...
        protected String doInBackground(String... params) {
            ...
            if (secondString.equals("part_used_info")) {
                ...
                postParameters = "sql=" + params[2];
            } else if (secondString.equals("load_Dipping_Feeder")) {
                postParameters = "use_for=" + params[2];
        ...
        protected void onPostExecute (String result){
            ...
            if (result == null){
                ...
                Toast.makeText(SMT_Parts_Use.this, "서버에 접속 할 수 없습니다.\n상세 내용은 로그를 참조 하십시오.", Toast.LENGTH_SHORT).show();
            } else {
            ...
            try {
                ...
                if (header.equals("part_used_info")) {
                    ...
                    if ((orgQty-usedQty-lossQty) == 0 ) {
                        ...
                        if (etFeederNo.length() == 0) {
                            tvStatus.setText("Feeder No. 스캔하여 주십시오.");
                        }
                    }
                    ...
                    if (!item.getString("Result").equals("Success")){
                        Toast.makeText(SMT_Parts_Use.this, mJsonString, Toast.LENGTH_SHORT).show();
                        return;
                    } else {
                        long[] pattern = {100,500,100,500,100,500};
                        vibrator.vibrate(pattern, -1); // miliSecond, 지정한 시간동안 진동
                        Intent intent = new Intent(SMT_Parts_Use.this, SMT_Working_Information.class);
                    ...
                    JSONObject item = jsonArray.getJSONObject(0);
                    if (item.getString("feeder_used").equals("0")) {
                        etFeederNo.setText(item.getString("feeder_no"));
                        tvStatus.setText("Part No, Lot No.를 스캔 하여 주십시오.");
                    } else {
                        tvStatus.setText("이미 사용 중인 Feeder No. 입니다.");
                    }
                    tempFeederNo = null;
                } else if (header.equals("feeder_status!")) {
                    etFeederNo.setText(tempFeederNo);
                    tvStatus.setText("Part No, Lot No.를 스캔 하여 주십시오.");
                    tempFeederNo = null;
                } else if (header.equals("dipping_feeder")) {
                    JSONArray jsonArray = jsonObject.getJSONArray("dipping_feeder");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject item = jsonArray.getJSONObject(i);
                        feederNoList = Add(feederNoList, item.getString("feeder_no"));
                    }
                    Log.d(activityTag, "Dipping Feeder List : " + Arrays.toString(feederNoList));
                } else if (header.equals("dipping_feeder!")) {
                    tvStatus.setText("사용가능한 Dipping Feeder가 없습니다.");
